...
現在の M5Core488 は、AR488 と GPIB 測定器を対象として作っています。
一方で、マクロ実行、ログ記録、タッチ UI、Web UI といった部分は、必ずしも GPIB 固有のものではありません。
AR488 は USB シリアル機器として扱っているため、将来的には他の USB シリアル機器に対しても、同じようなマクロ実行環境を応用できる可能性があります。
例えば、通信速度、改行コード、タイムアウトなどの通信条件をマクロやデバイスプロファイル側で定義できるようにすれば、より汎用的な USB Serial Macro Controller のような派生も考えられます。
また、USBTMC 対応測定器に対して、CoreS3 から直接 SCPI コマンドを送る方向も実験対象になりそうです。
これらは現時点では将来のアイデアであり、現在の M5Core488 は AR488 用として開発しています。
